export const projects = [
  {
    ...
    shortPitch:
      'A browser-based Minesweeper game with multiple difficulty levels, power-ups, undo, and dark mode, built with vanilla HTML, CSS, and JavaScript.',
    mainTechnologies: [
      'HTML5',
      'CSS3',
    ...
    architecturePattern:
      'Procedural JavaScript with global state (gGame, gLevel, gBoard); utility helpers in utils.js and game logic in game.js; DOM built via HTML string templates and updated with querySelector; inline event handlers in HTML (onclick, oncontextmenu).',
    dataPersistence: 'None (in-memory only; no localStorage, API, or backend)',
    elevatorPitch:
      'Built a feature-rich Minesweeper game in vanilla JavaScript as a solo front-end project. Implemented core game logic including mine placement, neighbor counting, flood-fill expansion, and win/loss detection across three board sizes. Extended the base game with hints, mega hint, safe-click, undo via deep-copied board state, lives, timer, dark mode, and audio feedback.',
    technicalChallenges: [
    ...
    myRole:
      'Sole developer of a client-side Minesweeper application (Sprint 1 coursework project, footer credits Adi Sabban).',
    keyStrengths: [
    ...
    constraintNote:
      'This is an honest assessment based on the five project files (index.html, css/app.css, js/game.js, js/utils.js, .vscode/settings.json). No React, TypeScript, build tools, or backend are used.',
  ...
  },
  {
    ...
    shortPitch:
      'A client-side meme editor where users pick images, add and style multiple text lines on a canvas, then save, download, or share creations.',
    mainTechnologies: [
      'HTML5',
      'CSS3',
      'Vanilla JavaScript',
    ...
    coreStack: {
      frontend:
        'Semantic HTML, modular CSS (@import for base, components, media queries), vanilla JS split into services and controllers; no build tool, npm, or framework.',
      backend: 'None',
      tools: 'VS Code',
    },
    keyFeatures: [
    ...
    architecturePattern:
      'Service–controller separation with global state (gMeme, gImgs): meme.service.js / gallery.service.js / storage.service.js hold data and logic; meme.controller.js / gallery.controller.js handle DOM, canvas rendering, and events; main.js handles multi-page SPA-style toggling via CSS classes.',
    dataPersistence:
      'localStorage (key: savedMemes, JSON via storage.service.js); static in-memory image catalog in gallery.service.js; one-off external POST for share upload (not a custom backend).',
    elevatorPitch:
      'Built a single-page Meme Generator using vanilla JavaScript and the Canvas API. Users browse a template gallery, edit multi-line captions with styling and drag positioning, and persist work in the browser. The app supports responsive layout, touch input, PNG export, and social sharing through an upload API integration.',
    technicalChallenges: [
    ...
    myRole:
      'Sole front-end developer on a sprint-style vanilla JS project — implemented architecture split (services/controllers), canvas editor, storage, and share/download flows end to end.',
    keyStrengths: [
  ...
  },
  {
    ...
    shortPitch:
      'A browser-based dual-app suite that clones Gmail inbox workflows and Google Keep notes, built as a pair sprint with no backend.',
    mainTechnologies: [
      'React (CDN)',
    ...
    coreStack: {
      frontend:
        'React components + service layer (mail.service.js) + shared async-storage.service.js (localStorage with simulated delay); no server, no TypeScript, no bundler (Vite/Webpack)',
      backend: 'None',
      tools: 'VS Code',
    },
    keyFeatures: [
    ...
    architecturePattern:
      'Component-based React with a service layer for data/CRUD; nested routes (MailIndex → MailList / MailDetails) and Outlet context to share list actions without prop drilling; HashRouter for client-side navigation',
    dataPersistence:
      'localStorage through async-storage.service.js (mailDB key); seed data generated on first load; no REST API (Axios is loaded globally but not used in the mail module)',
    elevatorPitch:
      'Co-developed KeepMailRead, a React single-page app mimicking Gmail and Google Keep. I owned the entire Gmail module end-to-end: routing, UI, service logic, and styling. The mail app supports multi-folder browsing, advanced filtering, compose/send, and a two-stage trash flow, with all state persisted in localStorage via a reusable async storage abstraction.',
    technicalChallenges: [
    ...
    myRole:
      'Frontend developer — Gmail module owner in a pair sprint; responsible for apps/mail/ (9 JSX components, mail.service.js, 8 CSS files) plus mail routes wired in the shared RootCmp.jsx. Partner built the notes app; shared shell includes header, storage utilities, and global styles.',
    keyStrengths: [
    ...
    constraintNote:
      'Scope note for interviews: Describe this as the Gmail half of a two-person sprint. The notes app (todos, images, YouTube API, trash/pin) is your partner\'s work unless you also contributed to shared files (RootCmp, AppHeader, async-storage.service.js).',
  ...
  },
  {
    ...
    shortPitch:
      'A full-stack vacation rental marketplace with search, maps, booking, and user accounts — built as a team of three.',
    mainTechnologies: [
      'React',
      'JavaScript (JSX)',
      'Redux',
    ...
    coreStack: {
      frontend:
        'React 18 + Vite, Redux, React Router v6, SCSS, Material UI, Axios',
      backend:
        'Node.js, Express, MongoDB (native driver), Socket.io, bcrypt, cookie-based auth',
      tools:
        'ESLint, Nodemon, Vite production build output to Abnb-backend/public',
    },
    keyFeatures: [
    ...
    architecturePattern:
      'Component-based React SPA with Redux modules (stay, user, order, system); backend follows Routes → Controllers → Services; dual local/remote service layer on the frontend (VITE_LOCAL toggles localStorage vs REST API); production SPA served by Express',
    dataPersistence:
      'MongoDB (stays, users, orders via REST API); localStorage in local dev mode; encrypted auth token in cookies',
    elevatorPitch:
    ...
    myRole:
      'Full-stack team contributor (3-person group) — shared ownership across frontend UI/state, backend API design, and feature integration. No git history is available to attribute specific files to you individually; frame this as collaborative full-stack delivery.',
    keyStrengths: [
    ...
    constraintNote:
      'Honesty notes for your portfolio: React: Yes — confirmed in package.json and .jsx source files. TypeScript: No — JavaScript/JSX only (@types/react is dev-only). Backend: Yes — Express + MongoDB is fully present. Group project: Call out "3-person team" on resume and in case studies; specify your personal contributions if you remember them (e.g., filters, maps, checkout, auth).',
